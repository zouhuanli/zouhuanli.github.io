...
# 三、流程总结
1.创建AnnotationConfigApplicationContext，注册AnnotatedBeanDefinitionReader和ClassPathBeanDefinitionScanner。
2.使用AnnotatedBeanDefinitionReader注册@Configuration自己作为Bean的信息到容器(DefaultListableFactory)和一些内部的解析器处理器。
3.进入刷新方法refresh,进入invokeBeanFactoryPostProcessors方法调用BeanFactory自己的后置处理器。
4.invokeBeanDefinitionRegistryPostProcessors方法，然后进入ConfigurationClassPostProcessor使用ConfigurationClassParser解析@Configuration配置类
5.ConfigurationClassPostProcessor解析配置类(如本文的appConfig),最终进入doProcessConfigurationClass解析@PropertySource、@ComponentScan、@Import、 @Bean等标签。
6.调用ClassPathBeanDefinitionScanner的doScan方法从扫描路径读取每个类，解析为ScannedGenericBeanDefinition，然后注册到DefaultListableFactory的beanDefinitionMap。
