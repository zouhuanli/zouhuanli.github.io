...
4.invokeBeanDefinitionRegistryPostProcessors方法，然后进入ConfigurationClassPostProcessor使用ConfigurationClassParser解析@Configuration配置类。<br>
5.ConfigurationClassPostProcessor解析配置类(如本文的appConfig),最终进入doProcessConfigurationClass解析@PropertySource、@ComponentScan、@Import、 @Bean等标签。<br>
6.调用ClassPathBeanDefinitionScanner的doScan方法从扫描路径读取每个类，解析为ScannedGenericBeanDefinition，然后注册到DefaultListableFactory的beanDefinitionMap。<br>
