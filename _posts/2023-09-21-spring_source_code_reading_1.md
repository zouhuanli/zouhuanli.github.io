...
<h1>一、Spring的起源</h1>
    ...
    不同的场景，并衍生出SpringBoot和Spring Cloud 两个大开发框架。
...
<h2>1.核心容器</h2>
    ...
    spring-context:在core和beans之上，拓展了BeanFactory，提供了资源绑定、国际化、容器生命周期、事件发布等功能。ApplicationContext，应
    用上下文是其中的核心类。ApplicationContext一般也是客户端启动Spring容器的类，客户端不会直接操纵BeanFactory。
![spring-core和spring-beans](../images/2023-09-21-spring_source_code_reading_1/core_and_beans.png)
    
    spring-expression：提供spring表达式。

<h2>2.Data Access/Integration（数据访问／集成）</h2>
